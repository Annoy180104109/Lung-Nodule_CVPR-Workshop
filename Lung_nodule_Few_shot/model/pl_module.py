# =========================
# pl_module.py  (FSCSModule)  FULL FILE (FIXED FOR MIXED SUPERVISION)
# =========================
import abc
import os
import numpy as np
...
class FSCSModule(pl.LightningModule, metaclass=abc.ABCMeta):
    ...
    For sup="pseudo":
      - model must set self._last_query_pmask_all = [B,way,H,W] probabilities (query pseudo prob for each way).
      - For TRAINING loss: the model can mix pseudo and GT using args.pseudo_ratio inside compute_objective().
      - For METRICS/VIS:
          * If GT exists (batch["query_mask"]), we use GT for IoU evaluation (recommended).
          * Otherwise fall back to binarized pseudo.
    """
    ...
    def _save_episode_vis_onefile(
        ...
        gt_mask_bin: torch.Tensor,   # [B,H,W] binary (GT or binarized pseudo)
    ...
    def _shared_step(self, batch, batch_idx, split):
        ...
        # --------- choose seg GT mask for METRICS/VIS + for compute_objective ----------
        if self.sup == "pseudo":
            ...
            pseudo_prob_gtway = pmask_all.gather(1, idx).squeeze(1)  # [B,H,W] in [0,1]

            # ✅ IMPORTANT FIX:
            # Use REAL GT mask whenever it exists (so compute_objective can truly mix GT and pseudo)
            if "query_mask" in batch:
                gt_mask_bin = batch["query_mask"].long()  # real GT
            else:
                # fallback if GT not provided
                if hasattr(self, "binarize_pseudo"):
                    gt_mask_bin = self.binarize_pseudo(pseudo_prob_gtway)
                else:
        ...
        # Training loss (mixing is handled inside model.compute_objective when sup=="pseudo")
        ...
        with torch.no_grad():
            ...
            # For IoU meter, ensure batch has a query_mask field (binary GT used for metrics)
