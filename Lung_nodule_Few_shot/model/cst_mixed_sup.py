...
class ClfSegTransformer(FSCSModule):
    """
    Key points for pseudo mode:
      - Pseudo mask is generated from SUPPORT GT mask region (masked prototype on support KEYS).
      - Query pixels are scored by cosine similarity to the prototype.
      - Score is converted to probability with sigmoid(temperature * sim).
      - Adaptive threshold is used for:
            (a) visualization/metrics (binarized pseudo GT)
            (b) optional confidence weighting if you want
    """
    ...
    def _forward_one_shot(self, batch, shot_idx=0):
        ...
        with torch.no_grad():
            ...
            if self.sup == "pseudo":
                ...
                # repeat query qkv across way so it matches support batch (B*way)
                qry_qkv = self._select_last_layer_qkv(qry_qkv)  # -> tensor [3,B,heads,tokens,dim_head] ideally
                spt_qkv = self._select_last_layer_qkv(spt_qkv)

                # now make query qkv have B*way in batch dim
                # expected: [3,B,heads,tokens,dim_head]  -> repeat B dim
                if qry_qkv.dim() != 5:
                    raise RuntimeError(f"Unexpected qry_qkv dim={qry_qkv.dim()} (expected 5). Got shape={tuple(qry_qkv.shape)}")
    ...
    def compute_objective(self, output_cls, output_masks, gt_cls, gt_mask, gt_way, lambda_cls=1.0):
        ...
        output_masks: [B,way,2,H,W]
        gt_cls:       [B] 0..4
        gt_mask:      [B,H,W] (for pseudo mode it might be binary for metrics; training uses pmask_all anyway)
        gt_way:       [B] 0..way-1
        """
        ...
        if self.sup == "pseudo":
            ...
            # confidence weighting (adaptive): downweight mid-prob pixels
            conf = (p - 0.5).abs() * 2.0  # [0,1]
            ...
            if (not self._dbg_printed_pseudo_stats) and self.training and (not hasattr(self, "trainer") or self.trainer.is_global_zero):
                ...
                with torch.no_grad():
                    ...
                    print("[PSEUDO] p(min/mean/max)=({:.3f}/{:.3f}/{:.3f}) conf(mean)={:.3f} thr(mean)={:.3f}".format(
                        p.min().item(), p.mean().item(), p.max().item(), conf.mean().item(), thr.mean().item()
                    ))
        else:
    ...
    def _select_last_layer_qkv(qkv):
        """
        Many ViT/DINO code paths return:
          - tensor qkv already (single layer), OR
          - list/tuple of per-layer qkv (take last).
        We return a tensor for the last layer.
        """
    ...
    def adaptive_threshold(self, p_bhw: torch.Tensor) -> torch.Tensor:
        ...
        if self.pseudo_thr_mode == "meanstd":
            ...
            # torch.quantile exists on recent torch
    ...
    def binarize_pseudo(self, p_bhw: torch.Tensor) -> torch.Tensor:
        """
        Returns binary pseudo mask [B,H,W] long(0/1) using adaptive threshold.
        """
        thr = self.adaptive_threshold(p_bhw)  # [B]
        thr = thr.view(-1, 1, 1)
    ...
    def generate_pseudo_score_from_support_mask(self, qry_qkv, spt_qkv, spt_mask, resize=(96, 96)):
        ...
        qry_key = qry_key[:, :, 1:, :]  # [B*way, heads, HW, dim_head]
        spt_key = spt_key[:, :, 1:, :]
        ...
        # "area+threshold" is more robust than nearest when masks are small
