...
class ClfSegTransformer(FSCSModule):
    """
    LIDC adaptation:
    - Classification: 5-class malignancy classification on QUERY image (output_cls: [B,5])
    - Segmentation: binary bg/fg, conditioned on supports (output_masks: [B,way,2,H,W])
    - Supports ANY way/shot
    - Pseudo supervision FIXED: stores query pseudo masks as [B,way,H,W]
    """
    ...
    def __init__(self, args):
        ...
        # pseudo masks cache (do NOT put in batch for DP safety)
        self._last_query_pmask_all = None  # [B,way,H,W] when sup="pseudo"
    ...
    def forward(self, batch):
        ...
        # query classification (independent of supports)
        with torch.no_grad():
    ...
    def _forward_one_shot(self, batch, shot_idx=0):
        ...
        support_imgs = batch["support_imgs"][:, :, shot_idx]  # [B,way,3,H,W]
        support_masks = None if self.sup == "pseudo" else batch["support_masks"][:, :, shot_idx]  # [B,way,H,W]

        spt_img = rearrange(support_imgs, "b n c h w -> (b n) c h w")  # [B*way,3,H,W]
        spt_mask = None if self.sup == "pseudo" else rearrange(support_masks, "b n h w -> (b n) h w")  # [B*way,H,W]
        qry_img = batch["query_img"]
        ...
        with torch.no_grad():
            ...
            if self.sup == "pseudo":
                ...
                # this repeats query qkv per-way internally
                qry_qkv = qry_qkv.repeat_interleave(way, dim=1)

                resize = (self.imgsize, self.imgsize) if self.training else self._get_org_hw(batch)

                spt_mask, qry_mask = self.generate_pseudo_mask(
                    qry_qkv, spt_qkv,
                    batch["query_class_presence"].flatten(),
                    thr=0.4, resize=resize
                )
                # qry_mask comes as [B*way, H, W] -> reshape to [B,way,H,W]
                qry_mask = qry_mask.view(B0, way, *qry_mask.shape[-2:])
                self._last_query_pmask_all = qry_mask
    ...
    # -------------------------
    # Objective: needs gt_way from pl_module
    # -------------------------
    def compute_objective(self, output_cls, output_masks, gt_cls, gt_mask, gt_way, lambda_cls=1.0):
        ...
        output_masks: [B,way,2,H,W]
        gt_cls:       [B] 0..4
        gt_mask:      [B,H,W] (may be dummy if you don't have query GT)
        gt_way:       [B] 0..way-1
        """
        # classification loss
        cls_loss = F.cross_entropy(output_cls, gt_cls.long())

        # select correct way slot for segmentation
        ...
        if self.sup == "pseudo":
            if self._last_query_pmask_all is None:
                raise RuntimeError(
                    "sup='pseudo' but _last_query_pmask_all is None. "
                    "Make sure forward() ran and generated pseudo masks."
                )

            # _last_query_pmask_all: [B,way,H,W] float 0/1
            idx2 = gt_way.view(B, 1, 1, 1).expand(-1, 1, H, W)
            pseudo_tgt = self._last_query_pmask_all.gather(1, idx2).squeeze(1)  # [B,H,W]
            seg_target = pseudo_tgt.long()
        else:
            seg_target = gt_mask.long()

        # debug print once
        if (not self._dbg_printed) and self.training:
            self._dbg_printed = True
            print("===== LOSS DEBUG =====")
            print("sup:", self.sup)
            print("gt_mask unique:", torch.unique(gt_mask).tolist())
            if self._last_query_pmask_all is not None:
                print("pseudo unique:", torch.unique(self._last_query_pmask_all).tolist())
            print("seg_target unique:", torch.unique(seg_target).tolist())
            print("======================")
    ...
    def generate_pseudo_mask(self, qry_qkv, spt_qkv, class_gt, resize=(400, 400), thr=0.4,
        ...
        self_corr = F.interpolate(self_corr, (self.imgsize, self.imgsize),
                                  mode="bilinear", align_corners=True).squeeze(1)
        self_corr_ret = (self_corr + 1.) * .5

        cros_corr = cros_corr.mean(dim=1, keepdim=True)
        cros_corr = F.interpolate(cros_corr, resize,
                                  mode="bilinear", align_corners=True).squeeze(1)
        cros_corr_ret = (cros_corr + 1.) * .5
    ...
    # # reuse validation logic/metrics
    #      return self.validation_step(batch, batch_idx)

    # def test_epoch_end(self, outputs):
    # # if you already have validation_epoch_end, reuse it
